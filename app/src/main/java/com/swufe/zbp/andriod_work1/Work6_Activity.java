...
*/
    ...
    private void setRates3() {
        List<HashMap<String,String>> rates=new ArrayList<HashMap<String,String>>();
        SharedPreferences sharedPreferences=getSharedPreferences("countryRate", Activity.MODE_PRIVATE);
        Map<String, Float> data =(Map<String, Float>)sharedPreferences.getAll();
            for (String country : data.keySet()) {
                HashMap<String, String> map = new HashMap<String, String>();
                map.put("country", country);
                map.put("rate", "" + data.get(country));
